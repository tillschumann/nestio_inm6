...
\pleasewrite{Jochen}{Is there already any citable reference for the vis.cortex
model? We can also say that this is a microcircuit since one area is just a
single microcircuit and then we cite the microcircuit paper?}
...
\pleasewrite{Till}{Is the ASCII driver using shared file I/O or
  task-local? Depending on that the problems with metadata change}
